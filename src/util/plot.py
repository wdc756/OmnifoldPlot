...
def _sei_process_weighted_nat(op, bins, num_bins, data_dir):
...
def _sei_process_weighted_syn(op, syn_data, data_dir):
...
def _sei_process_re_weighted_syn(op, syn_data, data_dir):
...
def _sei_plot_syn(op, syn, mask_b, nat_b, denom):
...
def _sei_plot_weighted_syn(op, weighted_syn, mask_b, nat_b, denom):
    if op.plot_weighted_syn_error:
...
def _sei_plot_re_weighted_syn(op, re_weighted_syn, mask_b, nat_b, denom):
    if op.plot_re_weighted_syn_error:
...
def _sei_plot_bins(op, num_bins, bin_idx, weighted_nat, syn, weighted_syn, re_weighted_syn, data_dir):
    for b in range(num_bins):
        ...
        _sei_plot_syn(op, syn, mask_b, nat_b, denom)
        _sei_plot_weighted_syn(op, weighted_syn, mask_b, nat_b, denom)
        _sei_plot_re_weighted_syn(op, re_weighted_syn, mask_b, nat_b, denom)
...
def plot_sei(op: data.PlotSEIOptions, data_dir: str):
    ...
    weighted_nat, bin_idx = _sei_process_weighted_nat(op, bins, num_bins, data_dir)
    ...
    for syn_d in range(op.num_syn_datasets):
        for syn_p in range(op.num_percent_deviations):
            ...
            weighted_syn = _sei_process_weighted_syn(op, syn_data, data_dir)
            re_weighted_syn = _sei_process_re_weighted_syn(op, syn_data, data_dir)
